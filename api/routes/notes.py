...
def list_notes(lead_id: int, db: PGConn = Depends(get_db)):
...
def add_note(lead_id: int, body: NoteCreate, db: PGConn = Depends(get_db)):
    _assert_lead(db, lead_id)
    with db.cursor() as cur:
        cur.execute(
            "INSERT INTO contact_notes (property_id, note) VALUES (%s, %s) "
            "RETURNING *",
            (lead_id, body.note),
        )
