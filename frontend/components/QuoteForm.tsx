...
import { createQuote, updateQuote } from '@/lib/api'
import type { Quote, QuoteCreate } from '@/lib/types'
...
interface Props {
  quote?: Quote | null
  /** Prefill for the create-from-lead flow (client info + property link). */
  prefill?: Partial<QuoteCreate> | null
  onSaved: () => void
  onClose: () => void
}
...
export function QuoteForm({ quote, prefill, onSaved, onClose }: Props) {
  // The form is mounted fresh per open, so quote/prefill can seed state directly.
  const [clientName, setClientName]   = useState(quote?.client_name ?? prefill?.client_name ?? '')
  const [clientPhone, setClientPhone] = useState(quote?.client_phone ?? prefill?.client_phone ?? '')
  const [clientEmail, setClientEmail] = useState(quote?.client_email ?? prefill?.client_email ?? '')
  const [clientAddr, setClientAddr]   = useState(quote?.client_address ?? prefill?.client_address ?? '')
  const [issueDate, setIssueDate]     = useState(quote?.issue_date ?? todayStr())
  const [validUntil, setValidUntil]   = useState(quote?.valid_until ?? validUntilDefault())
  const [taxRate, setTaxRate]         = useState(quote ? String(quote.tax_rate) : '0')
  const [notes, setNotes]             = useState(quote?.notes ?? '')
  const [items, setItems]             = useState<DraftItem[]>(quote?.line_items.length
    ? quote.line_items.map(li => ({ description: li.description, quantity: String(li.quantity), unit_price: String(li.unit_price) }))
    : [emptyItem()]
  )
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const body: QuoteCreate = {
        property_id: quote?.property_id ?? prefill?.property_id,
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px 0', flexShrink: 0 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 600, color: 'var(--color-ink-900)', margin: 0 }}>
            {quote ? `Edit ${quote.quote_number}` : 'New Quote'}
          </h2>
        ...
        <form onSubmit={handleSubmit} style={{ overflowY: 'auto', flex: 1, padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              <label className="t-label" style={{ display: 'block', marginBottom: 6 }}>Valid Until</label>
              <input type="date" value={validUntil} onChange={e => setValidUntil(e.target.value)} className="drawer-input" style={{ width: '100%', boxSizing: 'border-box' }} />
            </div>
          </div>
        ...
        <div style={{ borderTop: '1px solid var(--color-ink-200)', padding: '14px 20px', background: 'var(--color-paper)', flexShrink: 0 }}>
          ...
          <button type="button" disabled={saving} className="btn-primary" onClick={handleSubmit} style={{ width: '100%', height: 50, fontSize: 15, fontWeight: 600 }}>
