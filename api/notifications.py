...
def send_appointment_email(*, to_email: str, business_name: str, title: str,
                           starts_at, location: str | None, ics_text: str) -> None:
    """Email a calendar invite with the appointment details and an .ics attachment
    the recipient can add to their calendar in one tap."""
    if not email_configured():
        raise RuntimeError("Email is not configured (RESEND_API_KEY / RESEND_FROM_EMAIL)")
    import base64
    import resend
    ...
    when = f"{starts_at:%A, %B %d, %Y at %I:%M %p}"
    where = f'<p style="color:#555;margin:0 0 4px">Location: {location}</p>' if location else ""
    html = f"""
      <div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;color:#1a1a1a">
        <h2 style="margin:0 0 4px">{title}</h2>
        <p style="color:#555;margin:0 0 20px">with {business_name}</p>
        <p style="margin:0 0 4px"><strong>{when}</strong></p>
        {where}
        <p style="color:#888;font-size:13px;margin-top:20px">A calendar invite is attached.
           Reply to this email to reschedule.</p>
      </div>
    """
    ...
    resend.Emails.send({
        "from": RESEND_FROM_EMAIL,
        "to": [to_email],
        "subject": f"Appointment confirmed: {title} — {starts_at:%b %d, %I:%M %p}",
        "html": html,
        "attachments": [{"filename": "appointment.ics", "content": encoded}],
    })


def send_appointment_sms(*, to_phone: str, business_name: str, title: str,
                         starts_at, location: str | None) -> None:
    if not sms_configured():
        raise RuntimeError("SMS is not configured (TWILIO_* env vars)")
    from twilio.rest import Client
    client = Client(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
    ...
    body = (
        f"{business_name}: Your appointment '{title}' is confirmed for "
        f"{starts_at:%b %d at %I:%M %p}{where}. Reply to reschedule."
    )
    client.messages.create(body=body, from_=TWILIO_FROM_NUMBER, to=to_phone)
